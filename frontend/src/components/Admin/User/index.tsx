...
export default function User() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axiosInstance.get("http://localhost:2000/user", {
        ...
        withCredentials: true, // ⬅️ Ini penting supaya cookies dikirim otomatis
      });
      ...
      setToast({ type: "error", message: "Gagal mengambil data user." });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    console.log("Menghapus user dengan ID:", id); // Debug
    ...
    try {
      ...
      console.log("Gagal menghapus User:", error.response?.data || error.message);
