...
const TableCell: React.FC<TableCellProps> = ({
  ...
  if (isHeader) {
    return (
      <th 
        className={`${baseClasses} font-semibold text-gray-900 dark:text-gray-100 text-left ${className}`}
      >
        {children}
      </th>
    );
  ...
  return (
    <td 
      className={`${baseClasses} text-gray-700 dark:text-gray-300 ${className}`}
    >
      {children}
    </td>
  );
